# 002, builder-first emit path

## Context

Generators emit through JSX that the sync renderer lowers into `ConstNode`, `TypeNode`, and `FileNode`. Those same nodes can be built directly with the `create*` factories already exported from `@kubb/ast`, which removes the JSX hop for generators that do not need composition. This slice proves the builder path covers a real generator and produces identical output, which de-risks removing the reconciler in slice 003.
...
4. Write a short note that captures the builder-versus-JSX choice for the slice 006 documentation.
...
2. `pnpm boundaries` stays green and the generator module shows no `@kubb/renderer-jsx` import.
