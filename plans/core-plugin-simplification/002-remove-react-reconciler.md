...
2. Audit every plugin and example for legacy `jsxRenderer` usage, and confirm no component relies on React hooks or suspense. Migrate any holdout to the builder API first.
...
- `@kubb/plugin-swr`, the only consumer of the async runtime, moved to `jsxRenderer` first (commit `d6cbd15`, plugins #319). 17 swr tests pass, no snapshot changes.
...
- 55 renderer-jsx tests pass, typecheck clean, gzipped bundle from a 510 KiB budget down to about 7.4 kB.
- Follow-up: `jsxRendererSync` and `jsxRenderer` were a single function behind two names, so they collapse to one export named `jsxRenderer`. The plugins repo and the platform docs pipeline move off the `jsxRendererSync` name.
