...
- Slices 001 and 002 are sequenced. Prove the builder path first, then remove the reconciler.
- Slice 003 starts only after the emit path is stable, so the kit extraction does not fight the render change.
- Every slice verification re-runs the snapshot and example suite as the output-stability gate for AC-6.
