...
-   Replace `resolvers?: Array<ResolverTs>` with `resolver?: Partial<ResolverTs> & ThisType<ResolverTs>`. Supply only the methods you want to override; all others fall back to the active preset resolver.
-   Replace `transformers?: Array<Visitor>` with `transformer?: Visitor`.

#### [`@kubb/plugin-zod`](https://github.com/kubb-labs/kubb/tree/main/packages/plugin-zod)

-   Replace `resolvers?: Array<ResolverZod>` with `resolver?: Partial<ResolverZod> & ThisType<ResolverZod>`.
-   Replace `transformers?: Array<Visitor>` with `transformer?: Visitor`.

#### [`@kubb/plugin-cypress`](https://github.com/kubb-labs/kubb/tree/main/packages/plugin-cypress)

-   Replace `resolvers?: Array<ResolverCypress>` with `resolver?: Partial<ResolverCypress> & ThisType<ResolverCypress>`.
-   Replace `transformers?: Array<Visitor>` with `transformer?: Visitor`.
