...
- **[`plugin-pinia-colada`](/plugins/plugin-pinia-colada/)** <Badge type="tip" text="beta" /> - Initial release

  Add support for generating type-safe Pinia Colada hooks from OpenAPI specifications. This new plugin generates `useQuery` and `useMutation` hooks compatible with [@pinia/colada](https://pinia-colada.esm.dev/), a lightweight data-fetching library for Vue.js applications.
  ...
  ```typescript [Generated Hook]
  export function useFindPetsByStatus(
    params?: MaybeRefOrGetter<FindPetsByStatusQueryParams>,
    options: {
      query?: Partial<UseQueryOptions<FindPetsByStatusQueryResponse, ResponseErrorConfig>>,
      client?: Partial<RequestConfig>
    } = {}
  ): UseQueryReturnType<TData, TError> & { queryKey: TQueryKey } {
    ...
    return useQuery({
      ...
      queryFn: async ({ signal }) => {
        ...
        return findPetsByStatus(toValue(params), config)
      },
    })
  }
  ```
  ...
  **Features:**
  ...
  - `MaybeRefOrGetter` support for Vue 3 reactivity
  - Configurable query and mutation keys
  - Zod validation integration
  - Works seamlessly with `@kubb/plugin-client`

  **Resources:**
  - [Plugin Documentation](/plugins/plugin-pinia-colada/)
  - [Live Example](/examples/pinia-colada/)
  - [Pinia Colada Docs](https://pinia-colada.esm.dev/)

  > **Note:** This plugin is currently in beta. The API may change in future releases.
