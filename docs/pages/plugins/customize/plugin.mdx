...
The easiest way to get started is to use our [template on Github](https://github.com/stijnvanhulle/kubb-plugin-template) or clone the following repo [https://github.com/stijnvanhulle/kubb-plugin-template](https://github.com/stijnvanhulle/kubb-plugin-template).
...
type MyPluginOptions = PluginFactoryOptions<Options, false>
...
export default createPlugin<MyPluginOptions>(({ version }) => {
  return {
    ...
    async resolveId(fileName, directory, options) {
