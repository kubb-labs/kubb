...
describe("exists", () => {
  ...
  it("should return false for non-existing file", async () => {
    expect(await exists(path.join(existsTestDir, "nonexistent.txt"))).toBe(
      false,
    );
  });
...
describe("clean", () => {
  ...
  it("does not throw when path does not exist", async () => {
    await expect(
      clean(path.join(os.tmpdir(), "kubb-does-not-exist")),
...
describe("getRelativePath", () => {
  ...
  test("returns correct relative path for Windows-style paths", () => {
    ...
    expect(
      getRelativePath(
        "C:/Users/user/project/mocks",
        "C:\\Users\\user\\project\\mocks\\folder\\test.js",
      ),
    ).toBe("./folder/test.js");
  });
