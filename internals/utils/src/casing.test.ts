...
describe("casing", () => {
  test("camelCase", () => {
    ...
    expect(camelCase("tag.tag", { isFile: true, prefix: "create" })).toBe(
      "tag/createTag",
    );
    expect(camelCase("tag.tag", { isFile: true, suffix: "schema" })).toBe(
      "tag/tagSchema",
    );
  });
  ...
  test.each([
    ...
    expect(camelCase(input, options as Parameters<typeof camelCase>[1])).toBe(
      expected,
    );
  });
  ...
  test.each([
    ...
    [
      ...
      { isFile: true },
      "getEnterpriseConfigurationsIdV20250",
    ],
    ...
    "camelCase(%s, %o) -> %s (file mode dot handling)",
  ...
  test.each([
    ...
    "pascalCase(%s, %o) -> %s (file mode dot handling)",
    (input, options, expected) => {
      expect(
        pascalCase(input, options as Parameters<typeof pascalCase>[1]),
      ).toBe(expected);
    },
  );
  ...
  test("pascalCase", () => {
    ...
    expect(pascalCase("create tag.tag", { isFile: true })).toBe(
      "createTag/Tag",
    );
    expect(pascalCase("tag.tag", { isFile: true, prefix: "create" })).toBe(
      "tag/CreateTag",
    );
    expect(pascalCase("tag.tag", { isFile: true, suffix: "schema" })).toBe(
      "tag/TagSchema",
    );
  });
});
