...
function schemaOpt(option: OptionDefinition) {
  const [schema] = getCommandSchema([
    { name: "cmd", description: "Cmd", options: { opt: option } },
  ]);
...
describe("getCommandSchema", () => {
  ...
  describe("option flags", () => {
    it.each([
      {
        option: { type: "boolean" as const, description: "Watch" },
        expected: "--opt",
      },
      {
        option: { type: "string" as const, description: "Config" },
        expected: "--opt <opt>",
      },
      {
        option: {
      ...
      {
        option: { type: "string" as const, description: "Config", short: "c" },
        expected: "-c, --opt <opt>",
      },
    ])(
      'builds flags "$expected" for $option.type option',
      ({ option, expected }) => {
  ...
  describe("option fields", () => {
    it.each([
      ...
      expect(
        schemaOpt(option)?.[field as keyof ReturnType<typeof schemaOpt>],
      ).toEqual(expected);
    });

    it("omits default when not set", () => {
      expect(
        schemaOpt({ type: "string", description: "Config" }),
