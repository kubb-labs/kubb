...
New codes `KUBB_UNSUPPORTED_FORMAT` (warning) and `KUBB_DEPRECATED` (info) are emitted by the OAS adapter. The renderer, counts, and json report handle every severity.
