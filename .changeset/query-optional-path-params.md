...
**Fixed**: Path params in query function signatures (`queryOptions`, `queryKey`, `useQuery`/`createQuery`, `useInfiniteQuery`) are now optional (`petId?`), matching the runtime behavior of the `enabled` guard. A `!` non-null assertion is added where the param is passed to the underlying client call, since the `enabled` guard guarantees the value is non-null at that point.
