...
JSX rendering is now 2–4× faster with `jsxRendererSlim`

Two new optimisations in `@kubb/renderer-jsx` cut render time significantly with no changes required to existing generators.

**`jsxRendererSlim` — React-free recursive renderer**

A new `jsxRendererSlim` factory replaces React's fiber scheduler with a lightweight recursive tree walk. Because all Kubb components are pure functions, the full React work loop is unnecessary overhead. Swap `jsxRenderer` for `jsxRendererSlim` in any generator and it just gets faster:

```ts
import { jsxRendererSlim } from '@kubb/renderer-jsx'

export const myGenerator = defineGenerator({
  renderer: jsxRendererSlim, // was: jsxRenderer
  // ...
})
```
...
**`jsxRendererSlim().stream()` — process files as they are rendered**
