...
`formatCacheStorage` also double-checks that a cache-hit file still exists in the underlying storage before skipping the write, so a manually deleted or reverted generated file gets recreated instead of staying missing, and it now persists its manifest once per build instead of on every cache-miss write. `kubb generate --watch` also now serializes rebuilds so overlapping filesystem events can't run two builds concurrently against the same output directory.
