...
Add markdown components to `@kubb/renderer-jsx` (`Frontmatter`, `Heading`, `Paragraph`, `List`, `Callout`) for authoring `.md` files declaratively in JSX. `Callout` emits GitHub-style alert syntax (`> [!TIP]`) portable across GitHub, GitLab, VitePress, Obsidian, and MDX.
