...
#### Function-based clients (default)
...
export async function getPetById({ petId }: { petId: number }) {
  const res = await fetch({ method: 'GET', url: `/pet/${petId}` })
  return res.data
}
```
...
Generates classes with methods for each operation, grouped by tag:
...
export class Pet {
  ...
  async getPetById({ petId }: { petId: number }) {
    ...
    const res = await request({ method: 'GET', url: `/pet/${petId}`, ...requestConfig })
    return res.data
  }
}
```
...
- Class-based generation uses the `classClientGenerator` which implements the `Operations` pattern
- Operations are grouped by tag when `group.type: 'tag'` is specified
- Each tag generates a separate class file (e.g., `Pet.ts`, `Store.ts`, `User.ts`)
- Import management groups imports by source file to avoid duplicates when operations reference types from different files
- All existing options (parser, paramsType, dataReturnType) work with both function and class-based generation
