...
import { defaultSolidQueryResolvers } from './resolver.ts'
...
registerDefaultResolvers(pluginSolidQueryName, defaultSolidQueryResolvers)

// Register name resolver with core
registerNameResolver(pluginSolidQueryName, (name, type) => {
  if (type === 'type') {
...
registerPathResolver(pluginSolidQueryName, (baseName, mode, options, ctx) => {
  if (mode === 'single') {
