...
export type KubbGenerationEndContext = {
  ...
  files: Array<FileNode>
  /**
   * Scoped view over the files written by this build, backed by `config.storage`.
   *
   * Reads are async and delegate to the underlying storage backend, so generated source
   * bytes are not duplicated in memory. The view is limited to keys produced in this run —
   * files from previous builds are not visible.
   *
   * @example Read a single generated file
   * ```ts
   * const code = await storage.getItem('/src/gen/pet.ts')
   * ```
   *
   * @example Iterate all generated files
   * ```ts
...
export type BuildOutput = {
  ...
  error?: Error
  /**
   * Scoped view over every file written during this build, backed by `config.storage`.
   *
   * Reads delegate to the underlying storage backend so generated bytes are not held
   * in memory a second time. Only keys produced by this build are visible.
   *
   * @example Read back a generated file
   * ```ts
...
export type Kubb = {
  ...
  /**
   * Scoped view over every file written by the most recent `build()` or `safeBuild()` call,
   * backed by `config.storage`. Only available after the build completes.
   *
   * Reads are async and delegate to the underlying storage backend — generated bytes are not
   * duplicated in memory. Only keys produced by this build are visible; earlier builds are not.
   *
   * @example Read a single generated file after build
   * ```ts
   * const { storage } = await kubb.safeBuild()
   * const code = await storage.getItem('/src/gen/pet.ts')
   * ```
   *
   * @example Stream all generated file paths
   * ```ts
