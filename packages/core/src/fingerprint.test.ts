...
import { computeFingerprint, stableStringify } from './fingerprint.ts'
...
describe('stableStringify', () => {
  ...
  it('drops functions', () => {
    expect(stableStringify({ a: 1, fn: () => 0 })).toBe(stableStringify({ a: 1 }))
  })
})
...
describe('computeFingerprint', () => {
  ...
  it('returns the same key for identical inputs', async () => {
    const a = await computeFingerprint({ config: makeConfig(), adapterSource: dataSource, version })
    const b = await computeFingerprint({ config: makeConfig(), adapterSource: dataSource, version })
    expect(a).toBe(b)
  })
  ...
  it('changes when plugin order changes', async () => {
    ...
    const a = await computeFingerprint({ config: makeConfig({ plugins: plugins(['a', 'b']) }), adapterSource: dataSource, version })
    const b = await computeFingerprint({ config: makeConfig({ plugins: plugins(['b', 'a']) }), adapterSource: dataSource, version })
    expect(a).not.toBe(b)
  })

  it('changes when output options change', async () => {
    const a = await computeFingerprint({ config: makeConfig({ output: { path: 'a' } }), adapterSource: dataSource, version })
    const b = await computeFingerprint({ config: makeConfig({ output: { path: 'b' } }), adapterSource: dataSource, version })
    expect(a).not.toBe(b)
  })
  ...
  it('changes when the kubb version changes', async () => {
    const a = await computeFingerprint({ config: makeConfig(), adapterSource: dataSource, version: '5.0.0' })
    const b = await computeFingerprint({ config: makeConfig(), adapterSource: dataSource, version: '5.1.0' })
    expect(a).not.toBe(b)
  })

  it('changes when the inline spec data changes', async () => {
    const a = await computeFingerprint({ config: makeConfig(), adapterSource: { type: 'data', data: { openapi: '3.1.0' } }, version })
    const b = await computeFingerprint({ config: makeConfig(), adapterSource: { type: 'data', data: { openapi: '3.0.0' } }, version })
    expect(a).not.toBe(b)
  })

  it('returns null for a URL input', async () => {
    const key = await computeFingerprint({ config: makeConfig(), adapterSource: { type: 'path', path: 'https://example.com/openapi.json' }, version })
    expect(key).toBeNull()
  })

  it('returns null when there is no adapter source', async () => {
    expect(await computeFingerprint({ config: makeConfig(), adapterSource: null, version })).toBeNull()
  })
  ...
  describe('path inputs', () => {
    ...
    it('changes when the spec file content changes', async () => {
      ...
      const a = await computeFingerprint({ config, adapterSource: { type: 'path', path: file }, version })

      await writeFile(file, '{"openapi":"3.0.0"}')
      const b = await computeFingerprint({ config, adapterSource: { type: 'path', path: file }, version })
