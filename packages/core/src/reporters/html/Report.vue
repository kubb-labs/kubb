...
const { generatedAt, report, pluginFiles } = data
  ...
  <main>
    <header class="hero">
      <div class="hero-heading">
        <div>
          <p class="eyebrow">Kubb run report</p>
          <h1>{{ report.name || 'Unnamed configuration' }}</h1>
        </div>
        <button class="theme-toggle" type="button" :aria-pressed="theme === 'dark'" @click="toggleTheme">
          {{ theme === 'dark' ? 'Light mode' : 'Dark mode' }}
        </button>
      </div>
      <p class="report-date">Generated {{ formatDate(generatedAt) }}</p>
    </header>
    ...
    <section>
      ...
      <dl class="stats">
        ...
        <div>
          <dt>Files</dt>
          <dd>{{ report.filesCreated }} generated</dd>
        </div>
        <div>
          <dt>Issues</dt>
          <dd>{{ report.counts.errors }} errors · {{ report.counts.warnings }} warnings · {{ report.counts.infos }} infos</dd>
        </div>
        <div>
          <dt>Duration</dt>
      ...
      <p v-if="report.plugins.failed.length" class="failure">Failed plugins: {{ report.plugins.failed.join(', ') }}</p>
      <div v-if="slowestPlugin" class="insight">
        <span>Slowest plugin</span><strong>{{ slowestPlugin.plugin }}</strong
        ><span>{{ formatMs(slowestPlugin.durationMs) }}</span>
      </div>
      <div class="output">
        <span>Output directory</span><code>{{ report.output }}</code>
      </div>
    </section>

    <section v-if="pluginActivity.length">
      <div class="section-heading">
        <div>
          <p class="eyebrow">Plugins</p>
          <h2>What each plugin wrote</h2>
        </div>
        <span class="count">{{ pluginActivity.length }} plugins</span>
      </div>
      <table class="plugin-table">
        ...
        <tbody>
          <tr v-for="activity in pluginActivity" :key="activity.plugin">
            <td>
              <code>{{ activity.plugin }}</code>
            </td>
            <td>{{ activity.files }}</td>
            <td>{{ activity.durationMs === undefined ? '—' : formatMs(activity.durationMs) }}</td>
            <td>
              <span class="status" :class="activity.failed ? 'failed' : 'success'">{{ activity.failed ? 'Failed' : 'Done' }}</span>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <section>
      ...
      <p v-if="!pluginFiles.length" class="muted">Kubb did not record any files for this run.</p>
      <details v-for="group in pluginFiles" :key="group.plugin" open>
        <summary>
          <span>{{ group.plugin }}</span
          ><span class="count">{{ group.files.length }}</span>
        </summary>
        <ul class="file-list">
          <li v-for="path in group.files" :key="path">
            <code>{{ path }}</code>
          </li>
        </ul>
      </details>
    ...
    <section v-if="report.diagnostics.length">
      <div class="section-heading">
        <div>
      ...
      <ul class="problems">
        <li v-for="diagnostic in report.diagnostics" :key="diagnostic.code + diagnostic.message">
          <strong>{{ diagnostic.severity }} · {{ diagnostic.code }}</strong>
          <span>{{ diagnostic.message }}</span>
          <small v-if="diagnostic.plugin">{{ diagnostic.plugin }}</small>
          <small v-if="diagnostic.location?.pointer">At {{ diagnostic.location.pointer }}</small>
          <p v-if="diagnostic.help">{{ diagnostic.help }}</p>
          <a v-if="diagnostic.docsUrl" :href="diagnostic.docsUrl" target="_blank" rel="noreferrer">Read the diagnostic guide</a>
        </li>
      </ul>
    </section>
  </main>
