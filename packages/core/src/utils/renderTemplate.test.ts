...
describe('renderTemplate', () => {
  test('template rendering', () => {
    ...
    expect(renderTemplate("{{ button: { position: 'relative' } }}", { fields: 'fields' })).toBe("{{ button: { position: 'relative' } }}")
    expect(renderTemplate("{{fields}} {{ button: { position: 'relative' } }}", { fields: 'fieldA' })).toBe("fieldA {{ button: { position: 'relative' } }}")
    expect(renderTemplate("{{ button: { position: 'relative' } }}{{fields}} ", { fields: 'fieldB' })).toBe("{{ button: { position: 'relative' } }}fieldB")
  })
})
