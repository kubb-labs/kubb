...
export async function setup(options: BuildOptions): Promise<SetupResult> {
  const { config: userConfig, events = new AsyncEventEmitter<KubbEvents>() } =
    options;
...
export async function safeBuild(
  ...
  const { fabric, pluginManager, events } = overrides
    ? overrides
    : await setup(options);
          ...
          .flatMap((file) => {
            ...
            return file.sources
              ?.map((source) => {
                ...
                const plugin = meta?.pluginKey
                  ? pluginKeyMap.get(JSON.stringify(meta.pluginKey))
                  : undefined;
