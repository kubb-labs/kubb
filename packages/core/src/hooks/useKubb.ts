...
type UseKubbReturn<TOptions extends PluginFactoryOptions = PluginFactoryOptions> = {
   ...
   */
  resolveBanner: (node?: RootNode) => string | null
  /**
   * Resolves the footer using the plugin's `output.footer` option.
   * Returns `null` when no footer is configured.
   * When `output.footer` is a function and no node is provided, returns `null`.
   */
  resolveFooter: (node?: RootNode) => string | null
}
  ...
  return {
    ...
    resolveFooter: (node?: RootNode) => {
      if (typeof output?.footer === 'function') {
        return node ? output.footer(node) : null
      }
