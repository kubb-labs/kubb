...
export async function loadUserConfig(
  ...
  { notify }: { notify: NotifyFunction },
): Promise<{ userConfig: Config; cwd: string }> {
  ...
  if (configPath) {
    ...
    try {
      ...
      await notify(
        NotifyTypes.CONFIG_LOADED,
        `Loaded config from ${configPath}`,
      );
    } catch (error) {
      await notify(
        NotifyTypes.CONFIG_ERROR,
    ...
    for (const configFileName of configFileNames) {
      try {
        ...
        await notify(
          NotifyTypes.CONFIG_LOADED,
          `Loaded ${configFileName} from current directory`,
        );
        break;
      } catch {
    ...
    if (!userConfig) {
      ...
      throw new Error(
        `No config file found. Please provide a config path or create one of: ${configFileNames.join(", ")}`,
      );
    }
  }
