...
function collectChildNodes(element: DOMElement): Array<CodeNode> {
  ...
  for (const child of element.childNodes) {
    ...
    if (child.nodeName === "kubb-function") {
      ...
      const value =
        textChild?.nodeName === TEXT_NODE_NAME
          ? (textChild as DOMNode<{ nodeName: "#text" }>).nodeValue
          : "";
      if (value) {
 ...
 * ```
 */
function squashSourceNodes(
  node: DOMElement,
  ignores: Array<ElementNames>,
): Set<SourceNode> {
    ...
    for (const child of current.childNodes) {
      ...
      if (child.nodeName === "kubb-source") {
        const source = createSource({
          ...
          isExportable: (child.attributes.get("isExportable") ??
            false) as boolean,
          isIndexable: (child.attributes.get("isIndexable") ??
            false) as boolean,
          nodes: collectChildNodes(child),
        });
...
export function processFiles(node: DOMElement): Array<FileNode> {
  ...
  function walk(current: DOMElement) {
    for (const child of current.childNodes) {
      ...
      if (
        child.nodeName !== TEXT_NODE_NAME &&
        child.nodeName !== "kubb-file" &&
      ...
      if (child.nodeName === "kubb-file") {
        if (child.attributes.has("baseName") && child.attributes.has("path")) {
          const sources = squashSourceNodes(child, [
            "kubb-export",
            "kubb-import",
          ]);
