...
const plainStringTypes = new Set<SchemaType>([
  "string",
  "uuid",
  "email",
  "url",
...
export function caseParams(
  ...
  return params.map((param) => {
    const transformed =
      casing === "camelcase" || !isValidVarName(param.name)
 ...
 * ```
 */
export function createDiscriminantNode({
  propertyName,
  value,
}: {
  propertyName: string;
...
function resolveParamsType({
  ...
  const groupName = groupLocation
    ? groupResolvers[groupLocation].call(resolver, node, param)
    : undefined;
 ...
 * ```
 */
export function createOperationParams(
  node: OperationNode,
  options: CreateOperationParamsOptions,
): FunctionParametersNode {
  const {
    paramsType,
    pathParamsType,
  ...
  const wrapTypeNode = (type: ParamsTypeNode): ParamsTypeNode =>
    type.kind === "ParamsType" && type.variant === "reference"
      ? wrapType(type.name)
      : type;
  ...
  const bodyType = node.requestBody?.schema
    ? wrapType(resolver?.resolveDataName(node) ?? "unknown")
    : undefined;
  ...
  if (paramsType === "object") {
    ...
    if (pathParams.length) {
      if (pathParamsType === "inlineSpread") {
        const spreadType =
          resolver?.resolvePathParamsName(node, pathParams[0]!) ?? undefined;
        params.push(
          createFunctionParameter({
            name: pathName,
        ...
        params.push(
          createParameterGroup({
            ...
            default:
              pathParamsDefault ??
              (pathChildren.every((c) => c.optional) ? "{}" : undefined),
          }),
        );
      }
    }
...
function buildGroupParam({
  ...
  if (groupType) {
    const type =
      groupType.type.kind === "ParamsType" &&
      groupType.type.variant === "reference"
        ? wrapType(groupType.type.name)
        : groupType.type;
    return [
      createFunctionParameter({ name, type, optional: groupType.optional }),
    ];
  }
  if (params.length) {
    return [
...
export function combineImports(
  ...
  const exportedNames = new Set(
    exports.flatMap((e) =>
      Array.isArray(e.name) ? e.name : e.name ? [e.name] : [],
    ),
  );
  const isUsed = (importName: string): boolean =>
    !source || source.includes(importName) || exportedNames.has(importName);
  ...
  for (const { node: curr } of keyed) {
    ...
    if (Array.isArray(name)) {
      name = [...new Set(name)].filter((item) =>
        typeof item === "string" ? isUsed(item) : isUsed(item.propertyName),
      );
      if (!name.length) continue;
 ...
 */
export function extractStringsFromNodes(
  nodes: Array<CodeNode> | undefined,
): string {
  ...
  return nodes
    .map((node) => {
      ...
      if ("generics" in node && node.generics)
        parts.push(
          Array.isArray(node.generics)
            ? node.generics.join(", ")
            : node.generics,
        );
