...
describe("VisitorContext — parent", () => {
  it("property visitor receives parent schema via context", () => {
    ...
    if (result.type === "object") {
      expect(result.properties.find((p) => p.name === "tag")?.required).toBe(
        true,
      );
      expect(result.properties.find((p) => p.name === "name")?.required).toBe(
        true,
      );
    }
  });
...
describe("ParentOf — type inference", () => {
  ...
  it("SchemaNode parent is a union of possible parents", () => {
    expectTypeOf<ParentOf<SchemaNode>>().toEqualTypeOf<
      | InputNode
      | OperationNode
      | SchemaNode
      | PropertyNode
  ...
  it("VisitorContext narrows parent for PropertyNode", () => {
    expectTypeOf<VisitorContext<PropertyNode>["parent"]>().toEqualTypeOf<
      SchemaNode | undefined
    >();
  });

  it("VisitorContext narrows parent for OperationNode", () => {
    expectTypeOf<VisitorContext<OperationNode>["parent"]>().toEqualTypeOf<
      InputNode | undefined
    >();
  });
  ...
  it("VisitorContext narrows parent for InputNode to undefined", () => {
    expectTypeOf<
      VisitorContext<InputNode>["parent"]
