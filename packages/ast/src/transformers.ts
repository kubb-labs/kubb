...
export function setDiscriminatorEnum({
  ...
  const hasProperty = objectNode.properties.some(
 ...
 * ```
 */
export function mergeAdjacentObjects(
...
export function simplifyUnion(members: Array<SchemaNode>): Array<SchemaNode> {
  const scalarPrimitives = new Set(
    members
      .filter((member) => isScalarPrimitive(member.type))
      .map((m) => m.type),
  );
  ...
  return members.filter((member) => {
    ...
    const enumValueCount =
      enumNode.namedEnumValues?.length ?? enumNode.enumValues?.length ?? 0;
    if (enumValueCount <= 1) {
      return true;
    }
    ...
    if (
      ...
      (scalarPrimitives.has("integer") || scalarPrimitives.has("number"))
    ) {
      return false;
    }
