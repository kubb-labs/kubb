...
describe("definePrinter", () => {
  ...
  it("dispatches print() to the matching node handler", () => {
    ...
    expect(printer.print(createSchema({ type: "string", min: 2 }))).toBe(
      "z.string().min(2)",
    );
  ...
  it("exposes resolved options on this.options inside handlers", () => {
    ...
    expect(
      zodPrinter({ prefix: "z" }).print(createSchema({ type: "string" })),
    ).toBe("z.string()");
    expect(
      zodPrinter({ prefix: "y" }).print(createSchema({ type: "string" })),
    ).toBe("y.string()");
    expect(zodPrinter().print(createSchema({ type: "string" }))).toBe(
      "z.string()",
    );
  });
  ...
  it("supports recursive this.transform() for object properties", () => {
    ...
    expect(zodPrinter().print(node)).toBe(
  ...
  it("supports recursive this.transform() for union members", () => {
    ...
    const zodPrinter = definePrinter<P>(() => ({
      ...
      nodes: {
        ...
        union(node) {
          const members =
            node.members?.map((m) => this.transform(m)).filter(Boolean) ?? [];
  ...
  it("infers the Printer type correctly", () => {
    ...
    expectTypeOf(printer.print(createSchema({ type: "string" }))).toEqualTypeOf<
