...
export const unpluginFactory: UnpluginFactory<Options | undefined> = (options, meta) => {
  ...
  async function runBuild(ctx: RollupContext) {
    ...
    const { error, failedPlugins, pluginTimings, files, outputStorage } = await kubb.safeBuild()
    ...
    await hooks.emit('kubb:generation:end', { config: resolvedConfig, files, outputStorage })
