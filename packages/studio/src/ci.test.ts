...
async function loadContext(provider: string) {
  vi.doMock('std-env', () => ({ detectProvider: () => ({ name: provider }), env: {}, isCI: true }))
...
describe('detectCIContext', () => {
  it('maps GitHub Actions metadata without forwarding the environment', () => {
    return loadContext('github_actions').then(({ detectCIContext }) =>
      expect(
        detectCIContext({
  ...
  it('maps GitLab merge request metadata', () => {
    return loadContext('gitlab').then(({ detectCIContext }) =>
      expect(
        detectCIContext({
  ...
  it('falls back to generic CI when std-env cannot identify a provider', () => {
    return loadContext('unknown-runner').then(({ detectCIContext }) => {
