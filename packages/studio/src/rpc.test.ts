import { describe, expect, it } from 'vitest'
import type { AgentApi } from './protocol/index.ts'
import { connectWebSocketRpc } from './rpc.ts'

const local = {} as AgentApi

describe('connectWebSocketRpc', () => {
  it('refuses a plaintext WebSocket to a remote host', async () => {
    await expect(connectWebSocketRpc({ url: 'ws://studio.example.com/s/1', token: 'secret', local })).rejects.toThrow(
      'Refusing unencrypted WebSocket to studio.example.com',
    )
  })

  it('refuses a scheme that is not a WebSocket at all', async () => {
    await expect(connectWebSocketRpc({ url: 'http://studio.example.com/s/1', token: 'secret', local })).rejects.toThrow('Refusing unencrypted WebSocket')
  })

  it.each(['ws://localhost:3000/s/1', 'ws://127.0.0.1:3000/s/1', 'ws://[::1]:3000/s/1'])('allows plaintext to the loopback host %s', async (url) => {
    const connection = await connectWebSocketRpc({ url, token: 'secret', local })
    connection.close()
  })
})
