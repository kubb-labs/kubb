...
describe('printConst', () => {
  ...
  it('handles string literals in value', () => {
    const node = createConst({ name: 'msg', nodes: ['"hello"'] })
    expect(printConst(node)).toBe('const msg = "hello";')
  })
})
...
describe('printType', () => {
  ...
  it('generates a simple type alias', () => {
    const node = createType({ name: 'PetStatus', export: true, nodes: ['string'] })
    expect(printType(node)).toBe('export type PetStatus = string;')
  })
  ...
  it('handles empty nodes', () => {
    ...
    expect(printType(node)).toBe('type Pet = unknown;')
  })
})
...
describe('printCodeNode', () => {
  ...
  it('dispatches Type nodes', () => {
    const node = createType({ name: 'Pet', nodes: ['string'] })
    expect(printCodeNode(node)).toBe('type Pet = string;')
  })
...
describe('printSource', () => {
  ...
  it('converts multiple nodes joining with newline', () => {
    const node = createSource({
      nodes: [createConst({ name: 'x', nodes: ['1'] }), createType({ name: 'Pet', nodes: ['string'] })],
    })
    expect(printSource(node)).toBe('const x = 1;\ntype Pet = string;')
  })
