...
export function print(...elements: Array<ts.Node>): string {
  ...
  const output = printer.printList(
    ...
    factory.createNodeArray(elements.filter(Boolean)),
    sourceFile,
  );
  ...
  const specifiers = name.map((item) => {
    if (typeof item === "object") {
      ...
      return factory.createImportSpecifier(
        false,
        alias ? factory.createIdentifier(propertyName) : undefined,
        factory.createIdentifier(alias ?? propertyName),
      );
    }
    return factory.createImportSpecifier(
      false,
...
export function printFunction(node: FunctionNode): string {
  ...
  const genericsStr = generics
    ? `<${Array.isArray(generics) ? generics.join(", ") : generics}>`
    : "";
  ...
  async parse(file, options = { extname: ".ts" }) {
    ...
    for (const item of (file as FileNode).imports) {
      const importPath = item.root
        ? getRelativePath(item.root, item.path)
        : item.path;
      ...
      importNodes.push(
        createImport({
          ...
          path:
            options?.extname && hasExtname
              ? `${trimExtName(importPath)}${options.extname}`
              : item.root
                ? trimExtName(importPath)
                : importPath,
    ...
    for (const item of (file as FileNode).exports) {
      ...
      exportNodes.push(
        createExport({
          ...
          path:
            options?.extname && hasExtname
              ? `${trimExtName(item.path)}${options.extname}`
              : trimExtName(item.path),
          isTypeOnly: item.isTypeOnly,
