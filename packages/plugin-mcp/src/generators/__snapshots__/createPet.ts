 ...
 * {@link /pets}
 */
export async function createPetsHandler({ data }: { data: CreatePetsMutationRequest }, request?: unknown): Promise<Promise<CallToolResult>> {
  const requestData = data

  const res = await fetch<CreatePetsMutationResponse, ResponseErrorConfig<Error>, CreatePetsMutationRequest>(
    { method: 'POST', url: `/pets`, data: requestData },
    request,
  )
  return {
    content: [
