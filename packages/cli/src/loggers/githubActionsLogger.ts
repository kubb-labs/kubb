  ...
  install(context, options) {
    ...
    context.on("generation:start", (config) => {
      const text = config.name
        ? `Generation for ${pc.bold(config.name)}`
        : "Generation";
    ...
    context.on("generation:end", (config) => {
      const text = getMessage(
        config.name
          ? `${pc.blue("✓")} Generation completed for ${pc.dim(config.name)}`
          : `${pc.blue("✓")} Generation completed`,
      );
    ...
    context.on("generation:summary", (config, { status, failedPlugins }) => {
      ...
      if (currentConfigs.length > 1) {
        closeGroup(
          config.name ? `Generation for ${pc.bold(config.name)}` : "Generation",
        );
      }
    });
  },
