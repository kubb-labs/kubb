...
export const clackLogger = defineLogger({
  ...
  install(context, options) {
    ...
    function showProgressStep() {
      ...
      if (totalFiles > 0) {
        parts.push(
          `Files ${pc.green(processedFiles.toString())}/${totalFiles}`,
        );
      }
    ...
    context.on("error", (error) => {
      ...
      if (
        isGenerating &&
        totalPlugins > 0 &&
        completedPlugins + failedPlugins < totalPlugins
      ) {
        failedPlugins++;
        showProgressStep();
      }
...
Run \`npm install -g @kubb/cli\` to update`,
    ...
    context.on("plugin:end", (plugin, { duration, success, error }) => {
      stopSpinner();
      ...
      const text = getMessage(success?
        `${pc.bold(plugin.name)} completed in ${pc.green(durationStr)}`:  `${pc.bold(plugin.name)} failed in ${pc.green(durationStr)}`,
      );
