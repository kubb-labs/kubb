 ...
 */
export function defineLoggerAdapter<TOptions extends LoggerAdapterOptions = LoggerAdapterOptions, TAdapter extends LoggerAdapter = LoggerAdapter>(
  build: (options: TOptions) => TAdapter,
): CreateLoggerAdapter<TOptions, TAdapter> {
  return (options) => {
