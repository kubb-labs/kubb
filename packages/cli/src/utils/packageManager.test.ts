...
describe("packageManager", () => {
  ...
  describe("hasPackageJson", () => {
    it.each([
      ...
      "returns $expected when existsSync returns $exists",
      ({ exists, expected }) => {
  ...
  describe("initPackageJson", () => {
    it.each<{ pm: PackageManagerInfo; expectedArgs: string[] }>([
      ...
      vi.mocked(spawn).mockReturnValue(
        makeChildProcess(0) as ReturnType<typeof spawn>,
      );
    ...
    it("rejects when the process exits with non-zero code", async () => {
      vi.mocked(spawn).mockReturnValue(
        makeChildProcess(1) as ReturnType<typeof spawn>,
      );
      ...
      await expect(initPackageJson("/tmp/project", pm)).rejects.toThrow(
        '"npm init -y" was terminated by signal undefined',
      );
    });

    it("rejects when spawn emits an error", async () => {
      vi.mocked(spawn).mockReturnValue(
        makeErrorChildProcess(new Error("spawn ENOENT")) as ReturnType<
          typeof spawn
        >,
      );
      const pm: PackageManagerInfo = {
  ...
  describe("installPackages", () => {
    ...
    it("rejects when the process exits with non-zero code", async () => {
      vi.mocked(spawn).mockReturnValue(
        makeChildProcess(2) as ReturnType<typeof spawn>,
      );
      const pm: PackageManagerInfo = {
        name: "pnpm",
        lockFile: "pnpm-lock.yaml",
      ...
      await expect(installPackages(["kubb"], pm)).rejects.toThrow(
        '"pnpm add kubb" was terminated by signal undefined',
      );
    });
  });
});
