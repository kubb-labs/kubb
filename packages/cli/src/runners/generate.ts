...
async function runToolPass({
  ...
  if (resolvedTool === "auto") {
    ...
    if (!detected) {
      ...
      await hooks.emit(
        "kubb:info",
        `Auto-detected ${toolLabel}: ${styleText("dim", resolvedTool)}`,
      );
    }
  }
  ...
  if (resolvedTool && resolvedTool !== "auto" && resolvedTool in toolMap) {
    ...
    try {
      const hookId = createHash("sha256")
        .update([configName, resolvedTool].filter(Boolean).join("-"))
        .digest("hex");
...
async function generate(options: GenerateProps): Promise<void> {
  ...
  const inputPath =
    input ??
    ("path" in options.config.input ? options.config.input.path : undefined);
  ...
  await hooks.emit(
    "kubb:info",
    config.name
      ? `Setup generation ${styleText("bold", config.name)}`
      : "Setup generation",
    inputPath,
  );
  ...
  await hooks.emit(
    "kubb:info",
    config.name
      ? `Build generation ${styleText("bold", config.name)}`
      : "Build generation",
    inputPath,
  );

  const { files, failedPlugins, pluginTimings, error, driver } =
    await kubb.safeBuild();
  ...
  if (config.output.format) {
    await runToolPass({
      ...
      noToolMessage:
        "No formatter found (oxfmt, biome, or prettier). Skipping formatting.",
  ...
  if (config.output.lint) {
    await runToolPass({
      ...
      noToolMessage:
        "No linter found (oxlint, biome, or eslint). Skipping linting.",
      configName: config.name,
      outputPath,
      logLevel,
      hooks,
...
export async function runGenerateCommand({
  ...
  watch,
}: GenerateCommandOptions): Promise<void> {
  const logLevel =
    logLevelMap[logLevelKey as keyof typeof logLevelMap] ?? logLevelMap.info;
      ...
      if (isInputPath(config) && watch) {
        await startWatcher([input || config.input.path], async (paths) => {
          ...
          clack.log.step(
            styleText(
              "yellow",
              `Watching for changes in ${paths.join(" and ")}`,
            ),
          );
        });
      } else {
