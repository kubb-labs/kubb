...
function generateConfigFile(
  ...
  const imports = selectedPlugins
    .map(
      (plugin) =>
        `import { ${plugin.importName} } from '${plugin.packageName}'`,
    )
    .join("\n");
  ...
  const pluginConfigs = selectedPlugins
    .map((plugin) => {
      const config =
        (pluginDefaultConfigs as Record<string, string>)[plugin.value] ??
        `${plugin.importName}()`;
      return `    ${config},`;
    })
    .join("\n");
...
export async function runInit({ yes, version }: InitOptions): Promise<void> {
  ...
  try {
    ...
    if (!hasPackageJson(cwd)) {
      ...
      packageManager = detectPackageManager(cwd);
      clack.log.info(
        `Detected package manager: ${styleText("cyan", packageManager.name)}`,
      );
    }
    ...
    if (yes) {
      selectedPlugins = availablePlugins.filter((plugin) =>
        (initDefaults.plugins as readonly string[]).includes(plugin.value),
      );
      clack.log.info(
        `Using plugins: ${styleText("cyan", selectedPlugins.map((p) => p.label).join(", "))}`,
      );
    } else {
      ...
      selectedPlugins = availablePlugins.filter((plugin) =>
        (selectedPluginValues as string[]).includes(plugin.value),
      );
    }

    // Ensure plugin-oas is always included
    if (!selectedPlugins.find((p) => p.value === "plugin-oas")) {
      selectedPlugins.unshift(
        availablePlugins.find((p) => p.value === "plugin-oas")!,
      );
    }
    ...
    const spinner = clack.spinner();
    spinner.start(
      `Installing ${packagesToInstall.length} packages with ${packageManager.name}`,
    );
    ...
    if (fs.existsSync(configPath)) {
      ...
      if (!yes) {
        ...
        if (clack.isCancel(shouldOverwrite) || !shouldOverwrite) {
          cancelAndExit(
            "Keeping existing configuration. Packages have been installed.",
          );
        }
      }
    ...
    clack.outro(
      styleText("green", "✓ All set!") +
        ...
        styleText(
          "cyan",
          `  1. Make sure your OpenAPI spec is at: ${inputPath}`,
        ) +
        "\n" +
    ...
    clack.log.error(
      styleText("red", "An error occurred during initialization"),
    );
