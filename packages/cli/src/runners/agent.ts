 ...
 */
function resolveAgentStartEnvironment({
  port,
  host,
  configPath,
  allowWrite,
  allowAll,
}: Omit<AgentStartOptions, "version">): ResolvedAgentStartEnvironment {
  const resolvedPort = port ?? process.env.PORT ?? agentDefaults.port;
  const resolvedHost =
    host !== agentDefaults.host
      ? host
      : (process.env.HOST ?? agentDefaults.host);
  const resolvedAllowAll =
    allowAll || process.env.KUBB_AGENT_ALLOW_ALL === "true";
  const resolvedAllowWrite =
    resolvedAllowAll ||
    allowWrite ||
    process.env.KUBB_AGENT_ALLOW_WRITE === "true";
  const agentRoot = process.env.KUBB_AGENT_ROOT ?? process.cwd();
  const agentConfigPath = path.resolve(
    process.cwd(),
    configPath || process.env.KUBB_AGENT_CONFIG || agentDefaults.configFile,
  );
  ...
  return {
    ...
    env: {
      ...
      KUBB_AGENT_RETRY_TIMEOUT:
        process.env.KUBB_AGENT_RETRY_TIMEOUT ?? agentDefaults.retryTimeout,
...
export async function runAgentStart({
  ...
  allowAll,
  version,
}: AgentStartOptions): Promise<void> {
  ...
  try {
    ...
    if (!Number.isInteger(numericPort) || numericPort <= 0) {
      throw new Error(
        `Invalid port "${resolvedEnv.port}". Provide a positive integer with --port or PORT.`,
      );
    }
    ...
    if (!resolvedEnv.allowWrite && !resolvedEnv.allowAll) {
      clack.log.warn(
        styleText(
          "yellow",
          "Filesystem writes disabled. Use --allow-write or --allow-all to enable.",
        ),
      );
    }
    ...
    if (!(await isPortAvailable(numericPort, resolvedEnv.host))) {
      clack.log.error(
        styleText(
          "red",
          `Port ${resolvedEnv.port} is already in use. Stop the existing process or choose a different port with --port.`,
        ),
      );
      process.exit(1);
    }
