...
describe('macroEnumName', () => {
  it('assigns the resolved name to enum nodes', () => {
    const node = ast.factory.createSchema({ type: 'enum', primitive: 'string', enumValues: ['a', 'b'] })
  ...
  it('strips names from boolean enum nodes', () => {
    const node = ast.factory.createSchema({ type: 'enum', primitive: 'boolean', enumValues: [true], name: 'ShouldDrop' })
