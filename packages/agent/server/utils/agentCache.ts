 ...
 */
export async function saveStudioConfigToStorage({
  sessionId,
  config,
}: {
  sessionId: string;
  ...
  await storage.setItem(key, [
    ...existing,
    { config, storedAt: new Date().toISOString() },
  ]);
}
 ...
 */
export async function getLatestStudioConfigFromStorage({
  sessionId,
}: {
  sessionId: string;
}): Promise<JSONKubbConfig | null> {
  const storage = getStorage();
  const key = `configs:${sessionId}`;
  const existing = await storage.getItem(key);
