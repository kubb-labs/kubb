...
export async function generate({
  ...
  await hooks.emit(
    "kubb:info",
    config.name ? `Setup generation ${config.name}` : "Setup generation",
  );
  ...
  await hooks.emit(
    "kubb:info",
    config.name ? `Build generation ${config.name}` : "Build generation",
  );
    ...
    if (formatter && formatter !== "auto" && formatter in formatters) {
      ...
      try {
        const hookId = createHash("sha256")
          .update([config.name, formatter].filter(Boolean).join("-"))
          .digest("hex");
        ...
        await hooks.onOnce("kubb:hook:end", async ({ success, error }) => {
          ...
          await hooks.emit(
            "kubb:success",
            `Formatting with ${formatter} successfully`,
          );
        });
      } catch (caughtError) {
  ...
  if (config.output.lint) {
    ...
    if (linter === "auto") {
      ...
      if (!detectedLinter) {
        ...
        await hooks.emit(
          "kubb:info",
          `Auto-detected linter: ${styleText("dim", linter)}`,
        );
      }
    }
    ...
    if (linter && linter !== "auto" && linter in linters) {
      ...
      try {
        const hookId = createHash("sha256")
          .update([config.name, linter].filter(Boolean).join("-"))
          .digest("hex");
        await hooks.emit("kubb:hook:start", {
          id: hookId,
