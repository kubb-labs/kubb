...
export async function publish({
  ...
  await hooks.emit(
    "kubb:info",
    `[publish] Running "${commandWithArgs}" in "${resolvedOutputPath}"`,
  );
  ...
  try {
    ...
    const error = new Error(
      `[publish] Failed to run "${commandWithArgs}": ${message}`,
    );
    error.cause = err;
  ...
  if (exitCode !== 0) {
    const error = new Error(
      `[publish] "${commandWithArgs}" exited with code ${exitCode}`,
    );
    await hooks.emit("kubb:error", error);
    throw error;
  }
  ...
  await hooks.emit(
    "kubb:success",
    `[publish] Published successfully in ${duration}ms`,
  );
}
