...
export function resolveStudioRuntimeConfig(
  ...
  return {
    ...
    retryInterval: parsePositiveIntegerEnv(
      env.KUBB_AGENT_RETRY_TIMEOUT,
      agentDefaults.retryIntervalMs,
    ),
    heartbeatInterval: parsePositiveIntegerEnv(
      env.KUBB_AGENT_HEARTBEAT_INTERVAL,
      agentDefaults.heartbeatIntervalMs,
    ),
    root,
    allowAll,
    ...
    poolSize: parsePositiveIntegerEnv(
      env.KUBB_AGENT_POOL_SIZE,
      agentDefaults.poolSize,
    ),
