...
export async function createAgentSession({
  token,
  studioUrl,
}: ConnectProps): Promise<AgentConnectResponse> {
...
export async function registerAgent({
  ...
  try {
    ...
    logger.success(
      `Agent registered with Studio with token ${maskString(token)}`,
    );
  } catch (error: any) {
    logger.error(
      "Failed to register agent with Studio",
