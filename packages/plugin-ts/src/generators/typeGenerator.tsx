...
function printResponseSchema({
  ...
  if (schemas.responses && schemas.responses.length > 0) {
    const responsesProperties: ts.PropertySignature[] = schemas.responses.map(
      (res) => {
        const identifier = pluginManager.resolveName({
          name: res.name,
          pluginKey: [pluginTsName],
    ...
    const responseNode = factory.createTypeAliasDeclaration({
      ...
      type: factory.createIndexedAccessTypeNode(
        factory.createTypeReferenceNode(
          factory.createIdentifier(`${baseName}Responses`),
          undefined,
        ),
...
export const typeGenerator = createReactGenerator<PluginTs>({
  ...
  Operation({ operation, generator, plugin }) {
    ...
    const mapOperationSchema = ({
      ...
      const schemaFromTree = tree.find(
        (item) => item.keyword === schemaKeywords.schema,
      );

      if (
        enumType === "asConst" &&
        schemaFromTree &&
        isKeyword(schemaFromTree, schemaKeywords.enum)
      ) {
      ...
      return (
        <>
          {mode === "split" &&
            imports.map((imp) => (
              <File.Import
                key={[name, imp.name, imp.path, imp.isTypeOnly].join("-")}
                root={file.path}
                path={imp.path}
                name={imp.name}
                isTypeOnly
    ...
    const combinedSchemaName =
      operation.method === "get" ? `${name}Query` : `${name}Mutation`;
  ...
  Schema({ schema, plugin, config }) {
    ...
    const schemaFromTree = schema.tree.find(
      (item) => item.keyword === schemaKeywords.schema,
    );
    ...
    return (
      ...
      >
        {mode === "split" &&
          imports.map((imp) => (
            <File.Import
              key={[schema.name, imp.path, imp.isTypeOnly].join("-")}
              root={resolution.file.path}
              path={imp.path}
              name={imp.name}
              isTypeOnly
            />
