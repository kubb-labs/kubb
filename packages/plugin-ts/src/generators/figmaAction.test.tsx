...
describe('figmaAction tests', async () => {
  const fabric = createReactFabric()

  test('Action schema from Figma spec', async () => {
    const oas = await parse(path.resolve('/tmp/figma_spec.yaml'))
    ...
    console.log('Tree for Action (Figma spec):')
    console.log(JSON.stringify(tree.map(t => ({keyword: t.keyword, ...('args' in t && Array.isArray((t as any).args) ? {args_len: (t as any).args.length} : {})})), null, 2))
    ...
    for (const file of [...fabric.context.fileManager.files.values()]) {
      ...
      for (const source of file.sources || []) {
        console.log('  Source:', { 
          ...
          value_preview: source.value.substring(0, 100)
        })
      }
    }
  })
