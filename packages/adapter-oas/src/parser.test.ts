...
describe("buildAst", () => {
  ...
  describe("schemas", () => {
    ...
    it("converts object schema with properties", async () => {
      ...
      expect(pet?.properties?.map((p) => p.name)).toEqual(
        expect.arrayContaining(["id", "name", "tag"]),
      );
    });
    ...
    it("flattens single-member allOf and propagates nullable", async () => {
      const oas = await buildMinimalOas();
      const root = parseOas(oas).root;
      const nullableString = root.schemas.find(
        (s) => s.name === "NullableString",
      );
    ...
    it("maps format date-time to datetime SchemaType", async () => {
      ...
      const createdAt = objectMember?.properties?.find(
        (p) => p.name === "createdAt",
      );
  ...
  describe("operations", () => {
    ...
    it("sets operationId, method, path, tags", async () => {
      const oas = await buildMinimalOas();
      const root = parseOas(oas).root;
      const listPets = root.operations.find(
    ...
    it("sets deprecated flag", async () => {
      const oas = await buildMinimalOas();
      const root = parseOas(oas).root;
      const createPet = root.operations.find(
    ...
    it("converts path parameters", async () => {
      const oas = await buildMinimalOas();
      const root = parseOas(oas).root;
      const getPet = root.operations.find(
        (op) => op.operationId === "getPetById",
      );
    ...
    it("leaves requestBody.required undefined when spec omits required", async () => {
      const oas = await buildMinimalOas();
      const root = parseOas(oas).root;
      const patchPet = root.operations.find(
        (op) => op.operationId === "patchPet",
      );
...
describe("parseSchema url", () => {
  ...
  it.each(["uri", "uri-reference", "url", "hostname", "idn-hostname"])(
    "maps format %s to url",
    (format) => {
...
describe("parseSchema allOf", () => {
  ...
  it("appends sibling properties as an extra intersection member", () => {
    ...
    expect(narrowed474?.members?.[narrowed474.members.length - 1]?.type).toBe(
      "object",
    );
  });
  ...
  it("resolves required keys missing from outer properties by looking into allOf members", () => {
    ...
    expect(injected?.properties?.find((p) => p.name === "id")?.required).toBe(
      true,
    );
  });
  ...
  it("does not inject required keys that are already in outer properties", () => {
    ...
    const objectMembers = narrowed510?.members?.filter(
      (m) => m.type === "object",
    );
...
describe("parseSchema oneOf / anyOf", () => {
  ...
  it("sets discriminatorPropertyName when discriminator is present", () => {
    ...
    expect(ast.narrowSchema(node, "union")?.discriminatorPropertyName).toBe(
      "petType",
    );
  });
  ...
  it("does not set discriminatorPropertyName when discriminator is absent", () => {
    ...
    expect(
      ast.narrowSchema(node, "union")?.discriminatorPropertyName,
    ).toBeUndefined();
  });
...
describe("parseSchema const (OAS 3.1)", () => {
  ...
  it("boolean const inside an object property has no name (inline literal)", () => {
    ...
    const objectNode = ast.narrowSchema(node, "object");
    const isHappyProp = objectNode?.properties.find(
      (p) => p.name === "isHappy",
    );
...
describe("parseSchema object properties", () => {
  ...
  it("required + not nullable → required: true, optional/nullish undefined", () => {
    ...
    const prop = ast
      .narrowSchema(node, "object")
      ?.properties?.find((p) => p.name === "id");
...
describe("parseSchema object inline enum naming", () => {
  it("enum name accumulates full parent path (OrderParamsStatusEnum)", () => {
    ...
    const statusProp = paramsSchema?.properties?.find(
      (p) => p.name === "status",
    );
...
describe("parseSchema tuple enum naming", () => {
  it("names enum elements inside a tuple property using parent + propName", () => {
    ...
    const identifierProp = narrowed?.properties?.find(
      (p) => p.name === "identifier",
    );
...
describe("parseSchema discriminator on union (oneOf/anyOf)", () => {
  ...
  it("narrows the discriminator property to a single value per union member when a mapping is present", () => {
    ...
    const topIntersection = ast.narrowSchema(node, "intersection")!;
    const unionNode = ast.narrowSchema(topIntersection.members?.[0], "union")!;
    const sharedPropertiesNode = ast.narrowSchema(
      topIntersection.members?.[1],
      "object",
    );
    const sharedTypeProp = sharedPropertiesNode?.properties?.find(
      (p) => p.name === "type",
    );

    expect(
      ast.narrowSchema(sharedTypeProp?.schema, "enum")?.enumValues,
    ).toEqual(["dog", "cat"]);
    ...
    const dogDiscNode = ast.narrowSchema(
      dogIntersection!.members![1],
      "object",
    );
    const dogTypeProp = dogDiscNode?.properties?.find((p) => p.name === "type");
    expect(dogTypeProp?.schema.readOnly).toBe(true);
    expect(ast.narrowSchema(dogTypeProp?.schema, "enum")?.enumValues).toEqual([
      "dog",
    ]);
    ...
    const catDiscNode = ast.narrowSchema(
      catIntersection!.members![1],
      "object",
    );
    const catTypeProp = catDiscNode?.properties?.find((p) => p.name === "type");
    expect(catTypeProp?.schema.readOnly).toBe(true);
    expect(ast.narrowSchema(catTypeProp?.schema, "enum")?.enumValues).toEqual([
      "cat",
    ]);
  });
  ...
  it("gives enum sibling properties a name derived from the union schema name", () => {
    ...
    const statusProp = sharedPropertiesNode?.properties?.find(
      (p) => p.name === "status",
    );
...
describe("parseSchema circular allOf discriminator detection", () => {
  it("skips allOf member that references the child schema back through its discriminator parent", async () => {
    ...
    expect(members.some((m) => m.type === "ref" && m.name === "Animal")).toBe(
      false,
    );
  });
  ...
  it("injects the narrowed discriminant value when the discriminator parent is filtered from allOf", async () => {
    ...
    const catDiscNode = catMembers.find((m) => {
      ...
      return obj?.properties?.some(
        (p) =>
          p.name === "type" &&
          ast.narrowSchema(p.schema, "enum")?.enumValues?.[0] === "cat",
      );
    });
    ...
    const dogDiscNode = dogMembers.find((m) => {
      ...
      return obj?.properties?.some(
        (p) =>
          p.name === "type" &&
          ast.narrowSchema(p.schema, "enum")?.enumValues?.[0] === "dog",
      );
    });
...
describe("buildAst", async () => {
  ...
  describe("operations", () => {
    it("listPets is a GET on /pets with a query parameter", () => {
      ...
      expect(
        op!.parameters.some((p) => p.name === "limit" && p.in === "query"),
      ).toBe(true);
    ...
    it("getPetById is a GET on /pets/:petId with a path parameter", () => {
      ...
      expect(
        op!.parameters.some((p) => p.name === "petId" && p.in === "path"),
      ).toBe(true);
    });
  });
...
describe("buildAst – parameter description propagation", async () => {
  ...
  it("propagates parameter-level description to schema.description for query params", () => {
    ...
    expect(limit?.schema.description).toBe(
...
describe("enum naming", () => {
  it("enum name accumulates full parent path without collision suffix", () => {
    ...
    const orderEnum = ast.narrowSchema(
      ast
        .narrowSchema(
          ast
            .narrowSchema(orderNode, "object")
            ?.properties?.find((p) => p.name === "params")?.schema,
          "object",
        )
    ...
    const customerEnum = ast.narrowSchema(
      ast
        .narrowSchema(
          ast
            .narrowSchema(customerNode, "object")
            ?.properties?.find((p) => p.name === "params")?.schema,
          "object",
        )
        ?.properties?.find((p) => p.name === "status")?.schema,
  ...
  it("oneOf shared property enums include schema name", () => {
    ...
    const topIntersection = ast.narrowSchema(node, "intersection");
    const sharedObj = topIntersection?.members?.find((m) =>
      ast
        .narrowSchema(m, "object")
        ?.properties?.some((p) => p.name === "status"),
    );
    const statusProp = ast
      .narrowSchema(sharedObj, "object")
      ?.properties?.find((p) => p.name === "status");
