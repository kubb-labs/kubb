...
describe("getDateType", () => {
  ...
  it("resolves date-time with dateType stringOffset", () => {
    expect(
      getDateType({ ...base, dateType: "stringOffset" }, "date-time"),
    ).toEqual({ type: "datetime", offset: true });
  });

  it("resolves date-time with dateType stringLocal", () => {
    expect(
      getDateType({ ...base, dateType: "stringLocal" }, "date-time"),
    ).toEqual({ type: "datetime", local: true });
  });
...
describe("resolveServerUrl", () => {
  it("returns the url unchanged when there are no variables", () => {
    expect(resolveServerUrl({ url: "https://api.example.com" })).toBe(
...
describe("getSchemas", () => {
  ...
  it("includes response schemas", () => {
    ...
    `);
    expect(nameMapping.get("#/components/responses/PetResponse")).toBe(
      "PetResponse",
    );
  });
  ...
  it("includes requestBody schemas", () => {
    ...
    `);
    expect(nameMapping.get("#/components/requestBodies/CreatePet")).toBe(
      "CreatePet",
    );
  });
  ...
  it("resolves three-way collision across all sources with semantic suffixes", () => {
    ...
    expect(nameMapping.get("#/components/schemas/Pet")).toBe("PetSchema");
    expect(nameMapping.get("#/components/responses/Pet")).toBe("PetResponse");
    expect(nameMapping.get("#/components/requestBodies/Pet")).toBe(
      "PetRequest",
    );
  });
  ...
  it("detects collisions from different casing that normalizes to the same PascalCase", () => {
    ...
    expect(nameMapping.get("#/components/schemas/pet_list")).toBe(
      "pet_listSchema",
    );
    expect(nameMapping.get("#/components/responses/PetList")).toBe(
      "PetListResponse",
    );
  });
  ...
  it("resolves collision between enum schema and response with semantic suffixes", () => {
    ...
    expect(nameMapping.get("#/components/responses/Status")).toBe(
      "StatusResponse",
    );
  });
