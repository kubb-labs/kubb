...
/**
 * Short PascalCase suffixes for the most common content types.
 * Used by `getContentTypeSuffix` to build distinct `operationId`s when
 * `expandContentTypes` generates one operation node per content type.
 */
...
/**
 * Returns a short PascalCase suffix derived from a content type string.
 *
 * Well-known content types map to a fixed label (e.g. `'multipart/form-data'` → `'Multipart'`).
 * Unknown types fall back to a PascalCase conversion of the subtype with any `x-` prefix removed.
 *
 * @example
 * ```ts
 * getContentTypeSuffix('application/json')              // 'Json'
 * getContentTypeSuffix('multipart/form-data')           // 'Multipart'
 * getContentTypeSuffix('application/x-www-form-urlencoded') // 'Form'
 * getContentTypeSuffix('application/vnd.api+json')      // 'VndApi+json'
 * ```
 */
export function getContentTypeSuffix(contentType: string): string {
  const mapped = CONTENT_TYPE_SUFFIX_MAP[contentType]
  if (mapped) return mapped

  // Fallback: take the subtype, strip the 'x-' vendor prefix, then PascalCase.
  const subtype = contentType.split('/')[1] ?? contentType
  return pascalCase(subtype.replace(/^x-/, ''))
}
