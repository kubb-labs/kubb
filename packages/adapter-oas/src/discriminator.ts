 ...
 * ```
 */
export function applyDiscriminatorInheritance(
  root: ast.InputNode,
): ast.InputNode {
  ...
  for (const schema of root.schemas) {
    ...
    if (!unionNode) {
      const intersectionMembers = ast.narrowSchema(
        schema,
        "intersection",
      )?.members;
    ...
    for (const member of members) {
      ...
      const enumValues = enumNode.enumValues.filter(
  ...
  return ast.transform(root, {
    schema(node, { parent }) {
      ...
      const existingIdx = objectNode.properties.findIndex(
        (p) => p.name === propertyName,
      );
      const newProperties =
        existingIdx >= 0
          ? objectNode.properties.map((p, i) =>
              i === existingIdx ? newProp : p,
            )
          : [...objectNode.properties, newProp];
