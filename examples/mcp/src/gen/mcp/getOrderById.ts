...
export async function getOrderByIdHandler({ orderId }: { orderId: GetOrderByIdPathParams["orderId"] }): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<GetOrderByIdQueryResponse, ResponseErrorConfig<GetOrderById400 | GetOrderById404>, unknown>({ method : "GET", url : `/store/order/${orderId}`, baseURL : "https://petstore.swagger.io/v2" })  
  return {
   content: [
     {
       type: 'text',
