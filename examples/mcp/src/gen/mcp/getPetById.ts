...
export async function getPetByIdHandler({ petId }: { petId: GetPetByIdPathParams["petId"] }): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<GetPetByIdQueryResponse, ResponseErrorConfig<GetPetById400 | GetPetById404>, unknown>({ method : "GET", url : `/pet/${petId}`, baseURL : "https://petstore.swagger.io/v2" })  
  return {
   content: [
     {
       type: 'text',
