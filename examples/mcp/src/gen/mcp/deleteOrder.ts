...
export async function deleteOrderHandler({ orderId }: { orderId: DeleteOrderPathParams["orderId"] }): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<DeleteOrderMutationResponse, ResponseErrorConfig<DeleteOrder400 | DeleteOrder404>, unknown>({ method : "DELETE", url : `/store/order/${orderId}`, baseURL : "https://petstore.swagger.io/v2" })  
  return {
   content: [
     {
       type: 'text',
