...
export async function deletePetHandler({ petId, headers }: { petId: DeletePetPathParams["petId"]; headers?: DeletePetHeaderParams }): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<DeletePetMutationResponse, ResponseErrorConfig<DeletePet400>, unknown>({ method : "DELETE", url : `/pet/${petId}`, baseURL : "https://petstore.swagger.io/v2", headers : { ...headers } })  
  return {
   content: [
     {
       type: 'text',
