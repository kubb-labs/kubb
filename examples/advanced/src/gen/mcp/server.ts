...
import { createPetsMutationRequestSchema, createPetsPathParamsSchema, createPetsQueryParamsSchema, createPetsHeaderParamsSchema } from "../zod/petsController/createPetsSchema.ts";
...
server.tool("createPets", "Make a POST request to /pets/{uuid}", { uuid: createPetsPathParamsSchema.shape['uuid'], data: createPetsMutationRequestSchema, headers: createPetsHeaderParamsSchema, params: createPetsQueryParamsSchema }, async ({ uuid, data, headers, params }) => {
...
server.tool("findPetsByStatus", "Multiple status values can be provided with comma separated strings", { stepId: findPetsByStatusPathParamsSchema.shape['step_id'] }, async ({ stepId }) => {
  return findPetsByStatusHandler({ stepId })
})
          
server.tool("findPetsByTags", "Multiple tags can be provided with comma separated strings. Use tag1, tag2, tag3 for testing.", { headers: findPetsByTagsHeaderParamsSchema, params: findPetsByTagsQueryParamsSchema }, async ({ headers, params }) => {
...
server.tool("updatePetWithForm", "Make a POST request to /pet/{petId}:search", { petId: updatePetWithFormPathParamsSchema.shape['petId'], params: updatePetWithFormQueryParamsSchema }, async ({ petId, params }) => {
  return updatePetWithFormHandler({ petId, params })
})
          
server.tool("deletePet", "delete a pet", { petId: deletePetPathParamsSchema.shape['petId'], headers: deletePetHeaderParamsSchema }, async ({ petId, headers }) => {
...
server.tool("uploadFile", "Make a POST request to /pet/{petId}/uploadImage", { petId: uploadFilePathParamsSchema.shape['petId'], data: uploadFileMutationRequestSchema, params: uploadFileQueryParamsSchema }, async ({ petId, data, params }) => {
...
server.tool("createUsersWithListInput", "Creates list of users with given input array", { data: createUsersWithListInputMutationRequestSchema }, async ({ data }) => {
...
server.tool("updateUser", "This can only be done by the logged in user.", { username: updateUserPathParamsSchema.shape['username'], data: updateUserMutationRequestSchema }, async ({ username, data }) => {
