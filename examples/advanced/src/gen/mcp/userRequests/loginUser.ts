...
export async function loginUserHandler({ params }: { params?: LoginUserQueryParams }): Promise<Promise<CallToolResult>> {
  ...
  const res = await fetch<LoginUserQueryResponse, ResponseErrorConfig<LoginUser400>, unknown>({ method : "GET", url : `/user/login`, baseURL : "https://petstore.swagger.io/v2", params })  
  return {
   content: [
     {
       type: 'text',
