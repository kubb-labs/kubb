...
export const getUserByNameQueryKeySWR = ({ username }: { username: GetUserByNamePathParams["username"] }) => [{ url: '/user/:username', params: {username:username} }] as const

export type GetUserByNameQueryKeySWR = ReturnType<typeof getUserByNameQueryKeySWR>

export function getUserByNameQueryOptionsSWR({ username }: { username: GetUserByNamePathParams["username"] }, config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  return {
    fetcher: async () => {
 ...
 * {@link /user/:username}
 */
export function useGetUserByNameSWR({ username }: { username: GetUserByNamePathParams["username"] }, options: 
{
