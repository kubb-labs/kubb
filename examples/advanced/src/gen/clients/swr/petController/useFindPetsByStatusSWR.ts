...
export const findPetsByStatusQueryKeySWR = ({ step_id }: { step_id: FindPetsByStatusPathParams["step_id"] }) => [{ url: '/pet/findByStatus/:step_id', params: {step_id:step_id} }] as const

export type FindPetsByStatusQueryKeySWR = ReturnType<typeof findPetsByStatusQueryKeySWR>

export function findPetsByStatusQueryOptionsSWR({ step_id }: { step_id: FindPetsByStatusPathParams["step_id"] }, config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  return {
    fetcher: async () => {
 ...
 */
export function useFindPetsByStatusSWR({ step_id }: { step_id: FindPetsByStatusPathParams["step_id"] }, options: 
{
