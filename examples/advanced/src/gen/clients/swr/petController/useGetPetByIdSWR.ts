...
export function getPetByIdQueryOptionsSWR({ petId }: { petId: GetPetByIdPathParams["petId"] }, config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  return {
    fetcher: async () => {
 ...
 */
export function useGetPetByIdSWR({ petId }: { petId: GetPetByIdPathParams["petId"] }, options: 
{
