 ...
 */
export async function findPetsByStatus({ step_id }: { step_id: FindPetsByStatusPathParams["step_id"] }, config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  const { client: request = fetch, ...requestConfig } = config  
  
  const res = await request<FindPetsByStatusQueryResponse, ResponseErrorConfig<FindPetsByStatus400>, unknown>({ method : "GET", url : getFindPetsByStatusUrl({ step_id }).url.toString(), ... requestConfig })  
