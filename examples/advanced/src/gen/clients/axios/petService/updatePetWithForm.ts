...
export async function updatePetWithForm({ petId, params }: { petId: UpdatePetWithFormPathParams["petId"]; params?: UpdatePetWithFormQueryParams }, config: Partial<RequestConfig> & { client?: typeof fetch } = {}) {
  ...
  const res = await request<UpdatePetWithFormMutationResponse, ResponseErrorConfig<UpdatePetWithForm405>, unknown>({ method : "POST", url : getUpdatePetWithFormUrl({ petId }).url.toString(), params, ... requestConfig })  
