...
export function findPetsByTagsQueryOptions<TData = FindPetsByTags["response"]>(params?: FindPetsByTags["queryParams"], options: FindPetsByTags["client"]["parameters"] = {}): SWRConfiguration<TData, FindPetsByTags["error"]> {
    return {
        fetcher: async () => {
 ...
 * @link /pet/findByTags
 */
export function useFindPetsByTags<TData = FindPetsByTags["response"]>(params?: FindPetsByTags["queryParams"], options?: {
    ...
    const query = useSWR<TData, FindPetsByTags["error"], [
        typeof url,
        typeof params
