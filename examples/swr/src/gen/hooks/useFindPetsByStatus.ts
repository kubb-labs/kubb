...
export function findPetsByStatusQueryOptions<TData = FindPetsByStatus["response"]>(params?: FindPetsByStatus["queryParams"], options: FindPetsByStatus["client"]["parameters"] = {}): SWRConfiguration<TData, FindPetsByStatus["error"]> {
    return {
        fetcher: async () => {
...
export function useFindPetsByStatus<TData = FindPetsByStatus["response"]>(params?: FindPetsByStatus["queryParams"], options?: {
    ...
    const query = useSWR<TData, FindPetsByStatus["error"], [
        typeof url,
        typeof params
