...
export const orderSchema = z.object({ "id": z.number().optional(), "petId": z.number().optional(), "quantity": z.number().optional(), "shipDate": z.string().datetime().optional(), "status": z.enum(["placed", "approved", "delivered"]).describe("Order Status").optional(), "http_status": z.union([z.literal(200), z.literal(400), z.literal(500)]).describe("HTTP Status's and item of this").optional(), "complete": z.boolean().optional() });
 ...
 export const customerSchema = z.object({ "id": z.number().optional(), "username": z.string().optional(), "address": z.array(z.lazy(() => addressSchema).schema).optional() });
 ...
 export const userSchema = z.object({ "id": z.number().optional(), "username": z.string().optional(), "firstName": z.string().optional(), "lastName": z.string().optional(), "email": z.string().optional(), "password": z.string().optional(), "phone": z.string().optional(), "userStatus": z.number().describe("User Status").optional() });
 ...
 export const petSchema = z.object({ "id": z.number().optional(), "name": z.string(), "category": z.lazy(() => categorySchema).schema.optional(), "photoUrls": z.array(z.string()), "tags": z.array(z.lazy(() => tagSchema).schema).optional(), "status": z.enum(["available", "pending", "sold"]).describe("pet status in the store").optional() });



 export const addPetRequestSchema = z.object({ "id": z.number().optional(), "name": z.string(), "category": z.lazy(() => categorySchema).schema.optional(), "photoUrls": z.array(z.string()), "tags": z.array(z.lazy(() => tagSchema).schema).optional(), "status": z.enum(["available", "pending", "sold"]).describe("pet status in the store").optional() });
...
export const findPetsByStatusQueryResponseSchema = z.array(z.lazy(() => petSchema).schema);
 ...
 export const findPetsByTagsQueryParamsSchema = z.object({ "tags": z.array(z.string()).describe("Tags to filter by").optional(), "page": z.string().describe("to request with required page number or pagination").optional(), "pageSize": z.string().describe("to request with required page size").optional() }).optional();
...
export const updatePetWithFormQueryParamsSchema = z.object({ "name": z.string().describe("Name of pet that needs to be updated").optional(), "status": z.string().describe("Status of pet that needs to be updated").optional() }).optional();
 ...
 * @description successful operation
 */
export const getOrderById200Schema = z.lazy(() => orderSchema).schema;
...
export const createUsersWithListInputMutationResponseSchema = z.lazy(() => userSchema).schema;
 ...
 export const loginUserQueryParamsSchema = z.object({ "username": z.string().describe("The user name for login").optional(), "password": z.string().describe("The password for login in clear text").optional() }).optional();
