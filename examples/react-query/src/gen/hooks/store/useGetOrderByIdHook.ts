...
import { queryOptions, useQuery } from "@tanstack/react-query";
 ...
 export const getOrderByIdQueryKey = ({ orderId }: {
...
async function getOrderByIdHook({ orderId }: {
 ...
 export function getOrderByIdQueryOptionsHook({ orderId }: {
    orderId: GetOrderByIdPathParams["orderId"];
}, config: Partial<RequestConfig> = {}) {
 ...
 * {@link /store/order/:orderId}
 */
export function useGetOrderByIdHook<TData = GetOrderByIdQueryResponse, TQueryData = GetOrderByIdQueryResponse, TQueryKey extends QueryKey = GetOrderByIdQueryKey>({ orderId }: {
    orderId: GetOrderByIdPathParams["orderId"];
}, options: {
