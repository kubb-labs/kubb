...
- Make the AST node vocabulary spec-neutral so adapters for non-OpenAPI specs (AsyncAPI, GraphQL, Prisma, Arazzo) map onto built-in nodes — the model stays closed and fully typed, no adapter-defined kinds.
  ...
  **Breaking (types):** code reading `OperationNode.method`/`path` must handle `undefined` (an OpenAPI operation always carries them — guard or assert). ([#3380](https://github.com/kubb-labs/kubb/pull/3380), [`d06344b`](https://github.com/kubb-labs/kubb/commit/d06344b64b6c2115880796573d919f2b65c43db7))
